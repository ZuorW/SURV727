...
The cities Alorton `head(city_ranking)[1,1]` has the highest search frequency for `loans`, followed by Rosemont `head(city_ranking)[2,1]`,and Coal City `head(city_ranking)[3,1]`.\
...
In the plot, the frequencies have a similar shape in the first half of 2020, but the pattern becomes unclear in the second half. The initial spike in search hits of both keywords understandably corresponds to near the beginning of the pandemic when everyone was required to wear masks and news about rising cases of death due to infections.
...
We found that Hebron `head(city_ranking2_deaths)[1,1]` has the highest search frequency for the keyword "deaths" followed by Camanche `head(city_ranking2_deaths)[2,1]` and Galena `head(city_ranking2_deaths)[3,1]`. For the keyword "masks", Geneva `head(city_ranking2_masks)[1,1]` has the highest search frequency followed by Winnetka `head(city_ranking2_masks)[2,1]` and Lanark `head(city_ranking2_masks)[3,1]`.
