...
```{r}
# Initialize an empty data frame to hold your results
descs <- data.frame(Community = character(),
...
for(i in east) {
  ...
  description <- src %>% 
    ...
    html_text(trim = TRUE) %>% 
    paste(collapse = ' ')
  desc_df <- as.data.frame(description)

  community <- gsub("_", " ", east)

# Bind this row onto the data frame
  descs <- rbind(descs, data.frame(Community = community, Description = description, stringsAsFactors = FALSE))
}
