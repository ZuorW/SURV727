...
```{r}
ui <- fluidPage(
    titlePanel("Weather Trends Visualization - Animated"),

    sidebarLayout(
        sidebarPanel(
            actionButton("playButton", "Play Animation"),
            actionButton("stopButton", "Stop Animation"),
            selectInput("variableInput", "Choose Variable:", choices = c("Daylight", "Snowfall")),
            textOutput("weekIndicator")
        ),
...
server <- function(input, output, session) {
    ...
    # Reactive value for the current week
    currentWeek <- reactiveVal(1)

    # Update the map based on the selected variable
    updateMap <- function(week, variable) {
        if (is.null(week)) {
            return(NULL)
        }
        ...
        if (variable == "Daylight") {
            map <- map %>% addCircleMarkers(~lng, ~lat, radius = ~weekavg_hrs / 10,
                                            color = "#FFA500", fillOpacity = 0.8)
        } else {
            map <- map %>% addCircleMarkers(~lng, ~lat, radius = ~weekavg_snow / 10,
                                            color = "#0000FF", fillOpacity = 0.8)
        }
    ...
    observeEvent(input$playButton, {
        ...
        currentWeek(isolate(currentWeek()) %% 52 + 1)  # Loop through weeks
    })

    # Observe the stop button
    observeEvent(input$stopButton, {
        currentWeek(NULL)  # Stop updating
    })

    # Display current week
    output$weekIndicator <- renderText({
        paste("Current Week:", currentWeek())
    })

    # Render the map
    output$map <- renderLeaflet({
        updateMap(currentWeek(), input$variableInput)
    })

    # Update the map reactively
    observe({
        if (!is.null(currentWeek())) {
            output$map <- renderLeaflet({ updateMap(currentWeek(), input$variableInput) })
        }
    })
}
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            selectInput("keywordInput", "Select Keyword:", choices = c("light therapy", "SAD", "winter blues")),
            hr(),
...
server <- function(input, output, session) {
    ...
    output$map <- renderLeaflet({
        leaflet() %>% addTiles() %>% setView(lng = -84.5555, lat = 42.7325, zoom = 6)
    })
    ...
    observe({
        ...
        filtered_data <- mich_df %>%
            filter(year == input$yearInput, week == input$weekInput)

        # Update snowfall and daylight sliders
        if (nrow(filtered_data) > 0) {
            updateSliderInput(session, "snowfallInput", value = median(filtered_data$weekavg_snow, na.rm = TRUE))
            updateSliderInput(session, "daylightInput", value = median(filtered_data$weekavg_hrs, na.rm = TRUE))
        }
    ...
    # Update text box with average values
    output$infoBox <- renderPrint({
        if (nrow(filtered_data) > 0) {
            info <- filtered_data %>%
                summarise("Average Light Therapy Searches" = mean(`light therapy`, na.rm = TRUE),
                          "Average SAD Searches" = mean(SAD, na.rm = TRUE),
                          "Average Winter Blues Searches" = mean(`winter blues`, na.rm = TRUE))
            return(info)
        }
    })
}
...
```         
                  setView(lng = -84.7430, lat = 43.3148, zoom =7) #Coordinates for Michigan
```
...
# Shiny UI
ui <- fluidPage(
    titlePanel("Animated Weather and Google Search Trends"),
...
server <- function(input, output, session) {
    ...
    # Reactive value to store the current week
    currentWeek <- reactiveVal(1)
    ...
    updateMap <- function() {
        ...
        pal <- colorNumeric(palette = "Blues", domain = mich_df$weekavg_snow)

        leaflet(data) %>%
            addTiles() %>%
            setView(lng = -84.7430, lat = 43.3148, zoom =6.3) %>%  # Coordinates for Michigan
            addCircleMarkers(~lng, ~lat, 
                             radius = ~weekavg_hrs / 10, 
                             color = ~pal(weekavg_snow),
                             popup = ~paste("<strong>City:</strong>", city,
                                            "<br><strong>Week:</strong>", week,
                                            "<br><strong>Average Weekly Snow (mm):</strong>", weekavg_snow,
                                            "<br><strong>Average Daily Daylight (hrs):</strong>", weekavg_hrs,
                                            "<br><strong>Google Searches for \"light therapy\":</strong>", light_therapy,
                                            "<br><strong>Google Searches for \"seasonal affective disorder\":</strong>", SAD,
                                            "<br><strong>Google Searches for \"winter blues\":</strong>", winter_blues),
                             fillOpacity = 0.8)
        # Make sure to return the Leaflet map object at the end of this block
    }
    
    # Observe the play button
    observeEvent(input$playButton, {
        invalidateLater(1000, session)  # Update the map every second (1000 milliseconds)
        currentWeek(isolate(currentWeek()) %% 52 + 1)  # Loop through weeks
    })
   ...
   # Render the map
    ...
    observe({
        if (!is.null(currentWeek())) {
            output$map <- renderLeaflet({ updateMap() })
        }
    })
}
...
output$map <- renderLeaflet({
    data <- mich_df %>%
        filter(city == input$cityInput, week == input$weekInput)
...
output$map <- renderLeaflet({  leaflet() %>%  addTiles() %>%  setView(lng = -83.7430, lat = 44.3148, zoom = 7) %>% # Coordinates for Michigan  addMarkers(data = mich_df, ~longitude, ~latitude,  popup = paste("Date: ", mich_df$date, "<br>", "City:", mich_df$city, "<br>",  "Weather: ", mich_df$average_daily_daylight, " hours of daylight, ", mich_df$average_daily_snow, " inches of snow", "<br>",  "Trends: Light Therapy - ", mich_df$light_therapy,", ","SAD - ", mich_df$seasonal_affective_disorder, ", ",  "Winter Blues - ", mich_df$winter_blues)) }) }
