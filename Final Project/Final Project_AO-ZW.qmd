...
other_weather_daylight <- weather_history(i,
  ...
  other_weather_daylight_nd <- other_weather_daylight_nd[, col_order2] #reorder columns
...
other_weather_snow <- weather_history(i,
  ...
  other_weather_snow_nd <- other_weather_snow_nd[, col_order_snow2] #reorder columns
...
library(ggplot2)
head(trends)
trends$Week = as.Date(trends$Week, "%m/%d/%y")
ggplot(data = trends, aes(x= Week, group = 1))+
  ...
  labs(title = "Search Frequencies for indicators of SAD from 2017-2023", y= "Search Frequency") 
# we could use this plot (A) to see the overal trend across years


# then zoom to a subsection of A to see the data from 2017
head(X17)
X17$Week = as.Date(X17$Week, "%m/%d/%y")
# use this to transform the date to R_readable ones
...
ggplot(data = X17, aes(x= Week, group = 1))+
  ...
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Search Frequencies for indicators of SAD in 2017", y= "Search Frequency") 
  


# then zoom to a subsection of A to see the data from 2020 to see potential impacts of covid-19 lockdown "Prolonged Search Intensities for Light Therapy"
head(covid)
covid$Week = as.Date(covid$Week, "%m/%d/%y")
# use this to transform the date to R_readable ones

ggplot(data = covid, aes(x= Week, group = 1))+
  geom_line(aes(y =Seasonale_affective_disorder, color = "Seasonal Affective Disorder"), size =1.2) +
  geom_line(aes(y =winter_blues, color = "Winter Blues"),size =1.2) +
  geom_line(aes(y =light_therapy, color = "Light Therapy"), size =1.2) +
  theme(axis.text.x = element_text(size=10))+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(title = "Search Frequencies for indicators of SAD in 2020 (covid)", y= "Search Frequency") 
