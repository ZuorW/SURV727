...
colnames(mich_df) 
```
Interactive visualization (not useful right now because of missing city data in gtrends)
```{r}
...
For gtrends (later)
```{r}
gtrendsR::gtrends_auth(client.email = "akarioya@umich.com", path = "path/to/credentials")
